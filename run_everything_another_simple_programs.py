    ...
    if not checkpoint(3):
        ...
        for i,flavor in enumerate(flavors):
            ...
            for filter in filter_modes:
                ...
                output_ext = tfidf_flavors[i] + ('_both' if filter == 'both' else '') + '_vec'
                ...
                train_files = test_files = glob.glob('%s%s*.%s' % (source_dir, os.sep, flavor))
                if feature_extraction.extractTFIDFWithVectorizer(train_files, test_files, max_features, output_ext):
